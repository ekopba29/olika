...
@section('content')
    @php $action = Request::segment(2) == 'create_for' ? route('cat.storeFor', ['user' => $user->id] ) : route('cat.update', ['cat' => $cat->id]) @endphp
    @php $name = $cat->name ?? old('name') @endphp
    @php $birth_date = date('d-m-Y',strtotime( $cat->birth_date ?? now())) ?? old('birth_date') @endphp
